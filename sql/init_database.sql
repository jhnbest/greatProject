...
-- 创建数据库
CREATE DATABASE NVR_MONITOR FILE '/dm8/data/NVR_MONITOR/dm.nvr_monitor.dbf' SIZE 500 AUTOEXTEND ON NEXT 50 MAXSIZE 20000;
...
-- 创建索引
-- ============================================
CREATE INDEX IF NOT EXISTS IDX_DEVICE_STATUS_DEVICE_ID ON NVR_DEVICE_STATUS(DEVICE_ID);
CREATE INDEX IF NOT EXISTS IDX_DEVICE_STATUS_TIME ON NVR_DEVICE_STATUS(CREATE_TIME);
CREATE INDEX IF NOT EXISTS IDX_ALARM_DEVICE_ID ON NVR_ALARM(DEVICE_ID);
CREATE INDEX IF NOT EXISTS IDX_ALARM_STATUS ON NVR_ALARM(STATUS);
CREATE INDEX IF NOT EXISTS IDX_ALARM_CREATE_TIME ON NVR_ALARM(CREATE_TIME);
CREATE INDEX IF NOT EXISTS IDX_OPER_LOG_USER ON SYS_OPER_LOG(USER_ID);
CREATE INDEX IF NOT EXISTS IDX_OPER_LOG_TIME ON SYS_OPER_LOG(CREATE_TIME);
...
-- 6. 创建示例设备
INSERT INTO NVR_DEVICE (ID, NAME, CODE, BRAND, MODEL, IP_ADDRESS, PORT, USERNAME, PASSWORD, COMPANY_ID, CHANNEL_COUNT, STATUS)
VALUES 
...
SELECT 
    ...
    (SELECT COUNT(*) FROM NVR_ALARM a WHERE a.DEVICE_ID = d.ID AND a.STATUS = 'PENDING') as PENDING_ALARM_COUNT
FROM NVR_DEVICE d
LEFT JOIN SYS_COMPANY c ON d.COMPANY_ID = c.ID;

-- 告警统计视图
CREATE VIEW VW_ALARM_STATISTICS AS
SELECT 
    d.ID DEVICE_ID,
    as d.NAME as DEVICE_NAME,
    d.IP_ADDRESS,
    c.NAME as COMPANY_NAME,
    COUNT(CASE WHEN a.STATUS = 'PENDING' THEN 1 END) as PENDING_COUNT,
    COUNT(CASE WHEN a.ALARM_LEVEL = 'HIGH' AND a.STATUS = 'PENDING' THEN 1 END) as HIGH_LEVEL_COUNT,
    COUNT(*) as TOTAL_COUNT
FROM NVR_DEVICE d
LEFT JOIN NVR_ALARM a ON d.ID = a.DEVICE_ID
LEFT JOIN SYS_COMPANY c ON d.COMPANY_ID = c.ID
GROUP BY d.ID, d.NAME, d.IP_ADDRESS, c.NAME;
...
-- 清理过期数据
CREATE OR REPLACE PROCEDURE SP_CLEANUP_EXPIRED_DATA AS
BEGIN
    -- 删除30天前的设备状态数据
    ...
    WHERE CREATE_TIME < ADD_MONTHS(SYSDATE, -6) AND STATUS = 'HANDLED';
    
    COMMIT;
END;

-- 更新设备在线状态
CREATE OR REPLACE PROCEDURE SP_UPDATE_DEVICE_ONLINE_STATUS AS
BEGIN
    UPDATE NVR_DEVICE d
    SET IS_ONLINE = 0
    WHERE NOT EXISTS (
        SELECT 1 FROM NVR_DEVICE_STATUS s 
        WHERE s.DEVICE_ID = d.ID 
        AND s.CREATE_TIME > ADD_MINUTE(SYSDATE, -5)
    );
...
-- 注意: 达梦数据库的定时任务需要使用DBMS_JOB包
-- 以下为创建定时任务的示例

BEGIN
    DBMS_JOB.SUBMIT(
        JOB => NULL,
        WHAT => 'SP_CLEANUP_EXPIRED_DATA;',
        NEXT_DATE => TRUNC(SYSDATE+1) + 2/24, -- 每天凌晨2点执行
        INTERVAL => 'TRUNC(SYSDATE+1) + 2/24' -- 每天执行一次
    );
    COMMIT;
END;
...
-- 数据验证
-- ============================================
SELECT '分公司数量: ' || COUNT(*) as COUNT_INFO FROM SYS_COMPANY;
SELECT '用户数量: ' || COUNT(*) as COUNT_INFO FROM SYS_USER;
SELECT '设备数量: ' || COUNT(*) as COUNT_INFO FROM NVR_DEVICE;
SELECT '配置项数量: ' || COUNT(*) as COUNT_INFO FROM SYS_CONFIG;
